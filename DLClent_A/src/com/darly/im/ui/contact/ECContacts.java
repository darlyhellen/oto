...
import com.darly.im.storage.AbstractSQLManager;
...
public class ECContacts implements Parcelable {
	...
	public ContentValues buildContentValues() {
		...
		values.put(AbstractSQLManager.ContactsColumn.SUBACCOUNT,
				this.subAccount);
	...
	public void setClientUser(ClientUser user) {
		...
		setRemark(ContactLogic.CONVER_PHONTO[ContactSqlManager.getIntRandom(4,
				0)]);
	}

	public void setClientUser(ClientUser user, String tel, String mark) {
		if (mark == null) {
			setClientUser(user);
		} else {
			if (tel != null) {
				addPhone(new Phone(tel, user.getUserName()));
			}
			setContactid(user.getUserId());
			setNickname(user.getUserName());
			setRemark(mark);
