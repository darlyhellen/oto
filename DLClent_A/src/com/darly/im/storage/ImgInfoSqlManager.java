...
public class ImgInfoSqlManager extends AbstractSQLManager {
	...
	private ImgInfoSqlManager() {
		Cursor cursor = sqliteDB().query(TABLES_NAME_IMGINFO, null, null, null, null, null, BaseColumn.ID + " ASC ");
		if ((cursor.getCount() > 0) && (cursor.moveToLast())) {
			column_index = 1 + cursor.getInt(cursor.getColumnIndex(BaseColumn.ID));
		}
		cursor.close();
	...
	public long updateImageInfo(ImgInfo imgInfo) {
		...
		try {
			String where = BaseColumn.ID + " = " + imgInfo.getId();
