 ...
 */
package com.darly.dlclent.widget.circularload;
...
public class FitChart extends View {
	...
	private void initializeView(AttributeSet attrs) {
		chartValues = new ArrayList<FitChartValue>();
	...
	private void readAttributes(AttributeSet attrs) {
		...
		valueStrokeColor = resources
				.getColor(R.color.default_chart_value_color);
		...
		if (attrs != null) {
			TypedArray attributes = getContext().getTheme()
					.obtainStyledAttributes(attrs, R.styleable.FitChart, 0, 0);
			strokeSize = attributes.getDimensionPixelSize(
					R.styleable.FitChart_strokeSize, (int) strokeSize);
			valueStrokeColor = attributes.getColor(
					R.styleable.FitChart_valueStrokeColor, valueStrokeColor);
			backStrokeColor = attributes.getColor(
					R.styleable.FitChart_backStrokeColor, backStrokeColor);
			int attrAnimationMode = attributes.getInteger(
					R.styleable.FitChart_animationMode, ANIMATION_MODE_LINEAR);
	...
	private void renderBack(Canvas canvas) {
		...
		path.addCircle(drawingArea.centerX(), drawingArea.centerY(),
				viewRadius, Path.Direction.CCW);
	...
	private void renderValue(Canvas canvas, FitChartValue value) {
		if (!isInEditMode()) {
			...
			Renderer renderer = RendererFactory.getRenderer(animationMode,
					value, drawingArea);
	...
	private float calculateSweepAngle(float value) {
		float chartValuesWindow = Math.max(minValue, maxValue)
	...
	private void playAnimation() {
		ObjectAnimator animator = ObjectAnimator.ofFloat(this, "animationSeek",
				1.0f, 1.0f);
