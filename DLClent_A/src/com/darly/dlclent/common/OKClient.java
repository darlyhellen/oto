...
/**
 * @author zhangyh2 OKClient 上午9:44:20 TODO
 */
public class OKClient {
	...
	public static void get(List<BasicNameValuePair> params, String url,
			Callback callBack) {
		// cookie enabled
		Builder der = new Request.Builder();
		StringBuilder build = new StringBuilder();
		build.append(url);
		if (params != null) {
			build.append("?");
			for (BasicNameValuePair item : params) {
				build.append(item.getName() + "=" + item.getValue() + "&");
			}
			build.deleteCharAt(build.length() - 1);
		}
		der.url(build.toString());
		...
		if (userToken != null) {
			...
			String key = new String(Base64.encode(k.getBytes(), Base64.CRLF))
					.trim();
			der.addHeader("token", token);
			der.addHeader("key", key);
			LogUtils.i(token + " [+++++] " + key);
		}
		Call call = client.newCall(der.build());
		// 请求加入调度
	 ...
	 * @param callback
	 */
	private void _getAsyn(List<BasicNameValuePair> params,String url, final ResultCallback callback) {
		Builder der = new Request.Builder();
		StringBuilder build = new StringBuilder();
		build.append(url);
		if (params != null) {
		...
		deliveryResult(callback, der.build());
	}
	...
	public static void getAsyn(List<BasicNameValuePair> params,String url, ResultCallback callback) {
		getInstance()._getAsyn(params,url, callback);
	}
