...
import com.darly.dlclent.widget.circle.CircleBitmapDisplayer;
 ...
 *         呼出Imageloader图片加载类，防止耦合过多。替换不方便。 单例一个图片加载类
...
public class ImageLoaderUtil {
	...
	private void initImageLoader() {
		...
		File cacheDir = StorageUtils.getOwnCacheDirectory(APP.getInstance(),
				"Act/Cache");
		...
		ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(
				...
				.diskCache(
						new UnlimitedDiscCache(cacheDir, null,
								CCPAppManager.md5FileNameGenerator))
				// 自定义缓存路径
				// 自定义缓存路径
	...
	public void load(String url, Loading loading) {
		...
		ImageLoader.getInstance().loadImage(url, getOptions(R.drawable.icon),
