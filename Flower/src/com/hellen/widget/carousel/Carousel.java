package com.hellen.widget.carousel;
 ...
 */
public class Carousel<CarouselModel> implements OnPageChangeListener,
		OnClickListener {

	public interface ClickCarouselistener {

		void clickCarousel(Object url);
	...
	private List<CarouselModel> data;
	 ...
	 * 图片给定尺寸的宽高比例。
	 */
	private static double ASPECTRATIO = 2.66;

	public Carousel(Context context, List<CarouselModel> data,
	...
	private void init() {
		...
		lp = new RelativeLayout.LayoutParams(ConsMVP.WIDTH.getLen(),
				(int) (ConsMVP.WIDTH.getLen() / ASPECTRATIO));
	...
	private void setData() {
		...
		if (lent <= 1) {
			...
			// 当两个图片时，循环加载一次。
			ArrayList<CarouselModel> datas = new ArrayList<CarouselModel>();
			for (int i = 0, l = data.size(); i < l; i++) {
				for (CarouselModel home : data) {
					datas.add(home);
				}
			}
			setCarou(datas);
	...
	private void setCarou(List<CarouselModel> data) {
		// TODO Auto-generated method stub
		...
		for (CarouselModel url : data) {
			ImageView iv = new ImageView(context);
			iv.setScaleType(ScaleType.FIT_XY);
			iv.setClickable(true);
			ImageLoaderUtil.getInstance().loadImageNor(url.getImgUrl(), iv);
			...
			slid.setBackgroundResource(R.drawable.ic_launcher);
			layout.addView(slid);
			slide.add(slid);
		}
	}
